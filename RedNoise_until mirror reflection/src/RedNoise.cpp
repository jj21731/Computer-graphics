...
glm::vec3 traceReflection(const glm::vec3 &intersectionPoint, const glm::vec3 &normal, const glm::vec3 &rayDirection, const std::vector<ModelTriangle> &triangles, int depth) {
    ...
    // 如果反射光线没有击中任何物体，则返回背景色或黑色
    if (reflectionIntersection.distanceFromCamera == std::numeric_limits<float>::infinity()) {
        return glm::vec3(0);
    }
    return glm::vec3(reflectionIntersection.intersectedTriangle.colour.red, reflectionIntersection.intersectedTriangle.colour.green, reflectionIntersection.intersectedTriangle.colour.blue);
}
...
//    glm::vec3 lightPosition(0.0f, 1.5f, 2.0f);
