...
std::unordered_map<std::string, glm::vec3> calculateVertexNormals(const std::vector<ModelTriangle>& triangles) {
    std::unordered_map<std::string, glm::vec3> vertexNormals;
    std::unordered_map<std::string, int> vertexCount;

    for (const auto& triangle : triangles) {
        glm::vec3 faceNormal = calculateNormal(triangle);

        for (const auto& vertex : triangle.vertices) {
            std::string vertexKey = vec3ToString(vertex);
            vertexNormals[vertexKey] += faceNormal;
            vertexCount[vertexKey]++;
        }
    }

    std::unordered_map<std::string, glm::vec3> averageNormals;
    for (const auto& pair : vertexNormals) {
        glm::vec3 avgNormal = pair.second / static_cast<float>(vertexCount[pair.first]);
        averageNormals[pair.first] = glm::normalize(avgNormal);
    }

    return averageNormals;
}

glm::vec3 calculateBarycentricCoordinates(const glm::vec3 &point, const ModelTriangle &triangle) {
...
// processInput函数
void handlevent(SDL_Event &event, DrawingWindow &window, glm::vec3 &cameraPosition,glm::mat3 &cameraOrientation, float delta,bool &autoRotate,bool &raytracingEnabled,glm::vec3 &lightPosition,bool &gouraudEnabled) {
    ...
    switch (event.type) {
        case SDL_KEYDOWN:
            switch (event.key.keysym.sym) {
                ...
                case SDLK_w:
                    cameraPosition.z -= delta; // 向前移动
                    break;
                case SDLK_s:
                    cameraPosition.z += delta; // 向后移动
                    break;
                case SDLK_a:
                    cameraPosition.x += delta; // 向右移动
                    break;
                case SDLK_d:
                    cameraPosition.x -= delta; // 向左移动
                    break;
                case SDLK_UP:
                    cameraPosition.y += delta; // 向上移动
                    break;
                case SDLK_DOWN:
                    cameraPosition.y -= delta; // 向下移动
                    break;
...
std::vector<glm::vec3> generateCircularAreaLightSamples(const glm::vec3 &lightPosition, float radius, int sampleCount) {
    std::vector<glm::vec3> samples;
    for (int i = 0; i < sampleCount; ++i) {
        // Generate a random point within a circle
        float angle = static_cast<float>(rand()) / static_cast<float>(RAND_MAX) * 2.0f * glm::pi<float>();
        float distance = radius * std::sqrt(static_cast<float>(rand()) / static_cast<float>(RAND_MAX));
        float x = distance * std::cos(angle);
        float y = distance * std::sin(angle);
        // Assume the light is spread along the X and Y axis
        samples.push_back(lightPosition + glm::vec3(x, y, 0));
    }
    return samples;
}
...
// 反射光线跟踪函数
glm::vec3 traceReflection(const glm::vec3 &intersectionPoint, const glm::vec3 &normal, const glm::vec3 &rayDirection, const std::vector<ModelTriangle> &triangles, int depth) {
    if (depth <= 0) {
        return glm::vec3(0); // 达到最大反射深度，停止追踪
    }

    glm::vec3 reflectionDirection = glm::normalize(rayDirection - 2 * glm::dot(rayDirection, normal) * normal);
    RayTriangleIntersection reflectionIntersection = getClosestValidIntersection(intersectionPoint + reflectionDirection * 0.001f, reflectionDirection, triangles);

    if (reflectionIntersection.distanceFromCamera == std::numeric_limits<float>::infinity()) {
        return glm::vec3(0); // 如果反射光线没有击中任何物体，则返回背景色或黑色
    }

    const auto &hitTriangle = reflectionIntersection.intersectedTriangle;
    return glm::vec3(hitTriangle.colour.red, hitTriangle.colour.green, hitTriangle.colour.blue); // 返回击中物体的颜色
}
////软阴影soft shadow
void drawRaytracedScene2(DrawingWindow &window, glm::vec3 cameraPosition, const std::vector<ModelTriangle> &triangles, float focalLength, const glm::mat3 &cameraOrientation, const glm::vec3 &lightPosition) {
    int imageWidth = window.width;
    int imageHeight = window.height;
    float aspectRatio = float(imageWidth) / float(imageHeight);
    // 实现环境照明
    float ambientThreshold = 0.2f; // 设置环境照明的最小阙值
    const float reflectivity = 0.8f; // 使用固定的反射率
    std::vector<glm::vec3> lightSamples = generateCircularAreaLightSamples(lightPosition, 0.1f, 100);
    ...
    for (int y = 0; y < imageHeight; y++) {
        for (int x = 0; x < imageWidth; x++) {
            ...
            if (intersection.distanceFromCamera != std::numeric_limits<float>::infinity()) {
                ...
                // Calculate initial color based on the triangle's own color
                glm::vec3 color = glm::vec3(triangle.colour.red, triangle.colour.green, triangle.colour.blue);

                // Calculate soft shadows
                float shadowIntensity = 0.0f;
                for (const auto& sample : lightSamples) {
                    glm::vec3 toSample = glm::normalize(sample - intersection.intersectionPoint);
                    RayTriangleIntersection shadowIntersection = getClosestValidIntersection(intersection.intersectionPoint + toSample * 0.001f, toSample, triangles, true);
                    if (shadowIntersection.distanceFromCamera == std::numeric_limits<float>::infinity() ||
                        shadowIntersection.distanceFromCamera > glm::length(sample - intersection.intersectionPoint)) {
                        shadowIntensity += 1.0f;
                    }
                }
                shadowIntensity /= lightSamples.size();
                color *= shadowIntensity; // Apply soft shadow effect

                // Compute the brightness based on distance
                float lightDistance = glm::length(lightPosition - intersection.intersectionPoint);
                float distanceBrightness = 1.0f / (4.0f * glm::pi<float>() * lightDistance * lightDistance);
                distanceBrightness = glm::clamp(distanceBrightness, 0.0f, 1.0f);

                // Compute the incidence brightness
                glm::vec3 toLight = glm::normalize(lightPosition - intersection.intersectionPoint);
                float incidenceBrightness = glm::max(glm::dot(triangle.normal, toLight), 0.0f);
                incidenceBrightness = glm::pow(incidenceBrightness, 0.5f); // 使用幂函数调整亮度


                // Combine distance and incidence brightness
                float combinedBrightness = distanceBrightness * incidenceBrightness;
                float finalBrightness = glm::max(combinedBrightness, ambientThreshold);
                color *= finalBrightness; // Apply combined brightness
                ...
                float specIntensity = glm::pow(glm::max(glm::dot(viewDirection, reflectDirection), 0.0f), 256.0f);
                glm::vec3 specularColor = glm::vec3(1.0f, 1.0f, 1.0f) * specIntensity;
                color += specularColor; // Add specular highlights
                color = glm::clamp(color, 0.0f, 255.0f);

                // Convert the color to pixel value
                uint32_t adjustedColor = (255 << 24) |
                                         (static_cast<uint8_t>(color.r) << 16) |
                                         (static_cast<uint8_t>(color.g) << 8) |
                                         static_cast<uint8_t>(color.b);
                ...
                RayTriangleIntersection shadowIntersection = getClosestValidIntersection(intersection.intersectionPoint + toLight * 0.001f, toLight, triangles, true);
                if (shadowIntersection.distanceFromCamera != std::numeric_limits<float>::infinity() && shadowIntersection.distanceFromCamera < lightDistance) {
                    adjustedColor = darkenColor(adjustedColor); // Apply hard shadows
                }
                ...
                uint32_t backgroundColor = (255 << 24); // Black background
                window.setPixelColour(x, y, backgroundColor);
            }
        }
    }
...
glm::vec3 calculateVertexIntensity(const glm::vec3& vertexNormal, const glm::vec3& lightDirection, const Colour& vertexColour, float ambientIntensity, float diffuseIntensity) {
    glm::vec3 ambientComponent = glm::vec3(vertexColour.red, vertexColour.green, vertexColour.blue) * ambientIntensity;
    float diffuseComponent = glm::max(glm::dot(vertexNormal, lightDirection), 0.0f) * diffuseIntensity;
    return ambientComponent + diffuseComponent * glm::vec3(vertexColour.red, vertexColour.green, vertexColour.blue);
}

void gouraud(DrawingWindow &window,
             const std::unordered_map<std::string, glm::vec3>& vertexNormals,
    ...
    for (int y = 0; y < imageHeight; y++) {
        for (int x = 0; x < imageWidth; x++) {
            ...
            if (intersection.distanceFromCamera != std::numeric_limits<float>::infinity()) {
                ...
                // 计算每个顶点的光照强度
                std::vector<glm::vec3> vertexIntensities;
                for (const auto& vertex : triangle.vertices) {
                    std::string vertexKey = vec3ToString(vertex);
                    glm::vec3 vertexNormal = vertexNormals.at(vertexKey);
                    glm::vec3 lightDir = glm::normalize(lightPosition - vertex);
                    vertexIntensities.push_back(calculateVertexIntensity(vertexNormal, lightDir, triangle.colour, ambientIntensity, diffuseIntensity));
                }

                // 使用重心坐标进行插值
                glm::vec3 barycentricCoordinates = calculateBarycentricCoordinates(intersection.intersectionPoint, triangle);
                ...
                glm::vec3 color = interpolatedIntensity; // 使用插值后的光照强度作为颜色
                uint32_t adjustedColor = (255 << 24) |
                                         (static_cast<uint8_t>(color.r) << 16) |
                                         (static_cast<uint8_t>(color.g) << 8) |
                                         static_cast<uint8_t>(color.b);
...
int main() {
    ...
    std::vector<ModelTriangle> cornellBoxTriangles = loadObjFile("cornell-box.obj", 0.35, materials);
    std::vector<ModelTriangle> sphereTriangles = loadObjFile("sphere.obj", 0.5, materials);
    glm::vec3 lightPosition(0.0f, 0.65f, 1.0f);
//    glm::vec3 lightPosition(0.0f, 0.4f, 1.0f);
    float ambientIntensity = 1.0;
    float diffuseIntensity = 0.5;
    std::unordered_map<std::string, glm::vec3> vertexNormals = calculateVertexNormals(sphereTriangles);
    ...
    bool gouraudEnabled = false; // 默认不启用Gouraud着色
    ...
    while (true) {
        ...
        while (window.pollForInputEvents(event)) {
            handlevent(event, window, cameraPosition, cameraOrientation, delta,autoRotate, raytracingEnabled,lightPosition,gouraudEnabled);
        ...
        if (shouldRedraw) {
            ...
            if (gouraudEnabled) {
                gouraud(window, vertexNormals, cameraPosition, sphereTriangles, focalLength, cameraOrientation, lightPosition, ambientIntensity, diffuseIntensity);
            } else if (raytracingEnabled) {
                drawRaytracedScene2(window,cameraPosition,cornellBoxTriangles,focalLength,cameraOrientation,lightPosition);
            } else {
                // 调用原来的渲染函数
                renderFilledTriangles(window, cameraPosition, cornellBoxTriangles, focalLength, cameraOrientation);
            }
